...
  const [YeuCauContainer, setYeuCauContainer] = useState(false);
...
    try {
      const res = await fetch(`${apiURL}/api/donvan`, { 
        headers: { Authorization: `Bearer ${token}` },
      });
...
    }
    if (eta < getMinDate()) { 
      setError("Thời gian giao dự kiến (ETA) không được ở trong quá khứ.");
      return;
    }

    const DonVanData = {
      IdKhachHang, 
      IdKhoBaiBatDau: IdKhoBatDau, 
      IdKhoBaiKetThuc: IdKhoKetThuc,
      ETA: eta, 
      TrangThai: "ChoXuLy",
    
      YeuCauContainer: YeuCauContainer 
    };
...
    try {
      const res = await fetch(`${apiURL}/api/donvan/delete/${id}`, { 
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
...
        <h3 className="mb-4">Tạo Đơn Vận Mới </h3>
...
            id="YeuCauContainer"
            checked={YeuCauContainer}
            onChange={(e) => setYeuCauContainer(e.target.checked)}
          />
          <label className="form-check-label" htmlFor="YeuCauContainer">
            Yêu cầu xe Container
          </label>
         
        </div>
...
              .sort((a, b) => {
             
                const isAComplete = a.TrangThai === 'HoanThanh';
                const isBComplete = b.TrangThai === 'HoanThanh';
                
...
                    <br/>
        
                    <small style={{ color: dv.TrangThai === 'HoanThanh' ? 'green' : 'black' }}>
                      Trạng thái: {dv.TrangThai}
                    </small>
                    <br/>
                    {dv.YeuCauContainer && <span className="badge bg-warning text-dark">Container</span>}
                  </div>
                  <button 
